...
}
{
  if (!match($1,"%")) {
    if (match($1,"#")) { # closing a stanza
      ...
      host="";
      ncpu=spd=s=nj=myj=nss=nxl=nlpd=0;
	...
	spd=$4;
      ...
      else { # line reports that host is unreachable
	host=$8;
      }
    }
...
function processStanza (stanza,host,ncpu,spd,load,njobs,userlist) 
{
 if (stanza>0) {
   ...
   if (ncpu==0) { # this is an unreachable host
     ...
     rcs=((ncpu-load)>0.95?"BGCOLOR=#00ff00":(nss>10?"BGCOLOR=#ff0000":(nxl>1?"BGCOLOR=#ff00dd":(nlpd>10?"BGCOLOR=#00aaee":""))));
     printf(dlf,rcs,host,ncpu,spd,load,nj,userstr);
   }
 }
}
